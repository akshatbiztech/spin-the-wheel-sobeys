...
exports.spinWheel = functions.https.onCall(async (data, context) => {
    ...
    if (!clientRequestId) {
        throw new functions.https.HttpsError("invalid-argument", "clientRequestId is required.");
    }
    const configSnap = await db.doc("wheelConfig/default").get();
    if (!configSnap.exists) {
        throw new functions.https.HttpsError("failed-precondition", "Wheel config missing.");
    }
    const config = configSnap.data();
    if (!config.segments || config.segments.length !== 8) {
        throw new functions.https.HttpsError("failed-precondition", "Wheel must have 8 segments.");
    }
    ...
    if (!existing.empty) {
        ...
        return {
            ...
            nextAllowedAt: doc.nextAllowedAt
                .toDate()
                .toISOString(),
        };
    }
